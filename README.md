...
```
├── index.html          # Основний HTML (single page app)
├── script.js           # Вся логіка (≈5100 рядків)
├── base.css            # Базова вёрстка каркаса (кольори — через перемінні)
├── themes.css          # Система тем (схеми оформлення, як на DLE)
...
Інтерфейс використовує схеми оформлення (як на движку DLE). Кожна тема — набір CSS-перемінних у `themes.css`, що перемикаються через атрибут `<body data-theme="...">`:

- **binance** (дефолт) — тёмна схема із золотим акцентом
- **light** — світла схема

Щоб додати нову тему: додайте блок `body[data-theme="назва"] { ... }` у `themes.css`, кнопку у `index.html` (секція «Вигляд» → «Тема оформлення») і назву у масив `AVAILABLE_THEMES` у `script.js`.

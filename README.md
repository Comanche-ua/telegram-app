...
- **🛒 План закупівель:** Окремий розділ у застосунку та у `planner.html` (вкладка «План закупівель») — позиції з КЕКВ, вартістю та типом угоди (Prozorro Market / пряма угода / тендер), синхронізація з Google Таблицею, підсумки за КЕКВ та типами угод, гейдж виконання плану
...
├── planner.html        # Планер на місяць (План роботи + План закупівель)
├── script.js           # Вся логіка (≈7500 рядків)
